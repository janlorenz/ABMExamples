...
to setup
  ...
  create-turtles new-links [
     set color ifelse-value (random-float 1 < minority-size) [orange] [blue]
  ]
  reset-ticks
end
...
to go
  ...
  create-turtles 1 [
    set color ifelse-value (random-float 1 < minority-size) [orange] [blue]
    let partner rnd:weighted-n-of new-links other turtles [(ifelse-value (color = [color] of myself) [h] [1 - h]) * count link-neighbors]
    create-links-with partner
    move-to one-of partner
    fd 8
  ]
  tick
  if layout? [ layout ]
...
"" ""
PENS
"majority" 1.0 0 -13345367 true "" "if not plot? [ stop ]\nlet max-degree max [count link-neighbors] of turtles\n;; for this plot, the axes are logarithmic, so we can't\n;; use \"histogram-from\"; we have to plot the points\n;; ourselves one at a time\nplot-pen-reset  ;; erase what we plotted before\n;; the way we create the network there is never a zero degree node,\n;; so start plotting at degree one\nlet degree 1\nwhile [degree <= max-degree] [\n  let matches turtles with [count link-neighbors = degree and color = blue]\n  if any? matches\n    [ set-plot-pen-color blue\n      plotxy log degree 10\n             log (count matches) 10 ]\n  set degree degree + 1\n]"
"minority" 1.0 0 -955883 true "" "if not plot? [ stop ]\nlet max-degree max [count link-neighbors] of turtles\n;; for this plot, the axes are logarithmic, so we can't\n;; use \"histogram-from\"; we have to plot the points\n;; ourselves one at a time\nplot-pen-reset  ;; erase what we plotted before\n;; the way we create the network there is never a zero degree node,\n;; so start plotting at degree one\nlet degree 1\nwhile [degree <= max-degree] [\n  let matches turtles with [count link-neighbors = degree and color = orange]\n  if any? matches\n    [ set-plot-pen-color orange\n      plotxy log degree 10\n             log (count matches) 10 ]\n  set degree degree + 1\n]"
...
"" ""
PENS
"all" 1.0 1 -13345367 true "" "if not plot? [ stop ]\nlet max-degree max [count link-neighbors] of turtles\nplot-pen-reset  ;; erase what we plotted before\nset-plot-x-range 1 (max-degree + 1)  ;; + 1 to make room for the width of the last bar\nhistogram [count link-neighbors] of turtles"
"minority" 1.0 1 -955883 true "" "if not plot? [ stop ]\nlet max-degree max [count link-neighbors] of turtles\nplot-pen-reset  ;; erase what we plotted before\nset-plot-x-range 1 (max-degree + 1)  ;; + 1 to make room for the width of the last bar\nhistogram [count link-neighbors] of turtles with [color = orange]"
...
"" ""
PENS
"majority" 1.0 0 -13345367 true "" "plot mean [count link-neighbors] of turtles with [color = blue]"
"minority" 1.0 0 -955883 true "" "plot mean [count link-neighbors] of turtles with [color = orange]"
...
363
avg degree majority
mean [count link-neighbors] of turtles with [color = blue]
3
1
11
...
410
avg degree minority
mean [count link-neighbors] of turtles with [color = orange]
3
1
11
...
162
257
new-links is the number of links a new node creates upon creation (1 is the original model)
10
0.0
1
...
1314
363
avg degree maj by avg degree
mean [count link-neighbors] of turtles with [color = blue] / mean [count link-neighbors] of turtles
3
1
11
...
1314
409
avg degree min by avg degree
mean [count link-neighbors] of turtles with [color = orange] / mean [count link-neighbors] of turtles
3
1
11
...
IT EXTENDS THE BASIC PREFERENTIAL ATTACHMENT MODEL WITH:
- Two types of agents from which one is in the minority
- A homophily-heterophily parameter which gives additional weight to connect to models of the same (or different type).

THE TEXT BELOW DESCRIBES THE BASIC MODEL FROM WHICH THIS MODEL IS DERIVED.
THIS MODEL DESCRIBES TEH MODEL IN:
